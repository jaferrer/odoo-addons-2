...
class PurchasePlanningImprovedConfig(models.TransientModel):
    ...
    def get_default_opmsg_min_late_delay(self):
        opmsg_min_late_delay = self.env['ir.config_parameter'].get_param(
            'purchase_planning_improved.opmsg_min_late_delay', default=1)
    ...
    def set_opmsg_min_late_delay(self):
        ...
        for record in self:
            config_parameters.set_param('purchase_planning_improved.opmsg_min_late_delay',
    ...
    def get_default_opmsg_min_early_delay(self):
        opmsg_min_early_delay = self.env['ir.config_parameter'].get_param(
            'purchase_planning_improved.opmsg_min_early_delay', default=7)
    ...
    def set_opmsg_min_early_delay(self):
        config_parameters = self.env['ir.config_parameter']
        for record in self:
            config_parameters.set_param('purchase_planning_improved.opmsg_min_early_delay',
