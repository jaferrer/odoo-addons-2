...
class loadCss(models.TransientModel):
    ...
    def load_list_css(self):
        ...
        for mod_path in mod_path_list:
            if os.path.lexists(mod_path + os.path.sep + self.pathmod):
                filepath = mod_path + os.path.sep + self.pathmod
                break

        onlyfiles = []
        if filepath:
            onlyfiles = [self.pathmod + '/' + f for f in listdir(filepath) if isfile(join(filepath, f))]
