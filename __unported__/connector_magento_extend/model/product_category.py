...
class magentoextendProductCategory(models.Model):
    _name = 'magentoextend.product.category'
    _inherit = 'magentoextend.binding'
    _inherits = {'product.category': 'openerp_id'}
    _description = 'magentoextend product category'
    ...
    magentoextend_parent_id = fields.Many2one(
        comodel_name='magentoextend.product.category',
        string='magentoextend Parent Category',
        ondelete='cascade', )
...
class CategoryAdapter(GenericAdapter):
    _model_name = 'magentoextend.product.category'
    _magentoextend_model = 'products/categories'
...
class CategoryBatchImporter(DelayedBatchImporter):
    ...
    def run(self, filters=None):
        ...
        _logger.info('search for magentoextend Product Category %s returned %s',
                     filters, record_ids)
...
@magentoextend
class ProductCategoryImporter(magentoextendImporter):
    _model_name = ['magentoextend.product.category']
...
class ProductCategoryImportMapper(ImportMapper):
    ...
    def parent_id(self, record):
        if record['product_category']:
            ...
            magentoextend_cat_id = binder.to_openerp(rec['parent'])
