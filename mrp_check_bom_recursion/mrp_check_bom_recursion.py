...
class MrpBom(models.Model):
    ...
    @api.multi
    def check_bom_recursion(self, products=None):
        ...
        for rec in self:
            for bom_line in rec.bom_line_ids:
                if bom_line.product_id in products:
                    raise exceptions.UserError(_(u"This BoM for product %s is recursive. Product %s found at "
                                                 u"least twice") % (rec.product_id.display_name,
                                                                    bom_line.product_id.display_name))
                for product_bom in self.env['mrp.bom'].search([('product_id', '=', bom_line.product_id.id)]):
                    product_bom.check_bom_recursion(products=products | bom_line.product_id)

    @api.model
    def create(self, vals):
        res = super(MrpBom, self).create(vals)
        res.check_bom_recursion()
        return res
...
class MrpBomLine(models.Model):
    ...
    def create(self, vals):
        ...
        res.bom_id.check_bom_recursion()
        return res
    ...
    def write(self, vals):
        ...
        for rec in self:
            if "product_id" in vals or "bom_id" in vals:
                rec.bom_id.check_bom_recursion()
        return res
