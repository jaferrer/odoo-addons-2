...
class CustomerGeneratedCsvFileSequenced(models.Model):
    ...
    def get_default_importation_options(self):
        ...
        return {
            ...
            u'headers': True,
            u'separator': ',',
            u'float_thousand_separator': ',',
            u'float_decimal_separator': '.',
            u'advanced': True,
        }
    ...
    def launch_importation(self):
        for rec in self:
            ...
            if rec.asynchronous:
                options[u'use_connector'] = True
                options[u'chunk_size'] = rec.chunk_size
                existing_attachment_ids = self.env['ir.attachment'].search([('res_model', '=', 'queue.job')]).ids
            importation_result = wizard.do(wizard.id, fields=eval(rec.fields_to_import), options=options)
            if rec.asynchronous:
