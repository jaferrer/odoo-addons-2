...
class CustomerFileToImport(models.Model):
    ...
    def save_generated_csv_file(self, model, fields_to_import, table_dict_result, sequence=0):
        self.ensure_one()
        file_path = str(tempfile.TemporaryFile().name) + '.csv'
        ...
        with open(file_path, 'w') as out_file:
            ...
            for record_id in table_dict_result:
                row = [table_dict_result[record_id].get(field_name, '') for field_name in fields_to_import]
                if 'id' not in fields_to_import:
                    row = [record_id] + row
                out_file_csv.writerow(row)
        with open(file_path, 'rb') as tdmpfile:
            self.env['customer.generated.csv.file'].create({
                'import_id': self.id,
                'model': model,
                'sequence': sequence,
                'generated_csv_file': base64.b64encode(tdmpfile.read()),
                'fields_to_import': str(field_names),
                'context_to_add': json.dumps(self.get_context_to_add())
            })
...
class CustomerGeneratedCsvFileSequenced(models.Model):
    ...
    def get_default_values_for_importation_wizard(self):
        ...
        return {
            ...
            'file': base64.decodebytes(self.csv_file),
    ...
    def launch_importation(self):
        for rec in self:
            ...
            rec.started = True
            default_values_for_importatio = rec.get_default_values_for_importation_wizard()
            context_to_add = json.loads(rec.context_to_add)
            ctx = self.env.context
            context_to_add.update(ctx)
            wizard = self.env['base_import.import'].with_context(context_to_add).create(default_values_for_importatio)
