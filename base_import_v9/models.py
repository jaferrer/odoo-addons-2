...
from openerp.addons.ndp_tools.mimetypes import guess_mimetype
...
class ir_import(orm.TransientModel):
    ...
    def _read_file(self, file_type, record, options):
        ...
        if req:
            raise ImportError(_("Unable to load \"{extension}\" file: requires Python module \"{modname}\"").format(
                extension=file_extension, modname=req))
        raise ValueError(
            _("Unsupported file format \"{}\", import only supports CSV, ODS, XLS and XLSX").format(file_type))
