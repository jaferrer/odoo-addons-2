...
class magentoextendResPartner(models.Model):
    _name = 'magentoextend2.res.partner'
...
class MagentoAddress(models.Model):
    ...
    magento_partner_id = fields.Many2one(comodel_name='magentoextend2.res.partner',
...
@magentoextend
class CustomerBatchImporter(DelayedBatchImporterV2):
...
@magentoextend
class CustomerImporter(magentoextendImporterV2):
    _model_name = ['magentoextend2.res.partner']
    ...
    def _after_import(self, binding):
        ...
        book = self.unit_for(PartnerAddressBook, model='magentoextend2.address')
...
class PartnerAddressBook(ConnectorUnit):
    ...
    """
    _model_name = 'magentoextend2.address'
    ...
    def import_addresses(self, magento_partner_id, partner_binding_id):
        ...
        for address_id, infos in addresses:
            importer = self.unit_for(magentoextendImporterV2)
    ...
    def _get_address_infos(self, magento_partner_id, partner_binding_id):
        ...
        for address_id in mag_address_ids:
            ...
            if magento_record.get('is_default_billing'):
                binding_model = self.env['magentoextend2.res.partner']
...
class AddressAdapter(GenericAdapter):
    _model_name = 'magentoextend2.address'
    _magentoextend_model = 'customers/addresses'
...
@magentoextend
class AddressImporter(magentoextendImporterV2):
    _model_name = ['magentoextend2.address']
    ...
    def _define_partner_relationship(self, data):
        ...
        assert partner_binding_id, ("AdressInfos are required for creation of "
                                    "a new address.")
        binder = self.binder_for('magentoextend2.res.partner')
