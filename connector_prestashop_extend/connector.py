...
def get_environment(session, model_name, backend_id):
    ...
    backend_record = session.env['prestashopextend.backend'].browse(backend_id)
...
class PrestashopextendBinding(models.AbstractModel):
    _name = 'prestashopextend.binding'
    ...
    prestashopextend_id = fields.Char(string='ID on prestashop')

    _sql_constraints = [
        ('prestashop_uniq', 'unique(backend_home_id, prestashopextend_id)',
         'A binding already exists with the same prestashop ID.'),
    ]


class PrestashopBindingOdoo(models.AbstractModel):
    _name = 'prestashopextend.binding.odoo'
    _inherit = 'prestashopextend.binding'
...
def add_checkpoint(session, model_name, record_id, backend_id):
    ...
    return checkpoint.add_checkpoint(session, model_name, record_id,
                                     'prestashopextend.backend', backend_id)
