...
class PrestashopextendBinding(Binder):
    """ Generic Binder for PrestashopextendCommerce """
...
class PrestashopextendBindingModelBinder(PrestashopextendBinding):
    ...
    _model_name = [
        ...
        'prestashopextend.res.country',
        'prestashopextend.shop',
        'prestashopextend.sale.order.state',
        'prestashopextend.procurement.group',
        'prestashopextend.procurement.order',
    ]
    ...
    def to_openerp(self, external_id, unwrap=False, browse=False):
        ...
        bindings = self.model.with_context(active_test=False).search(
            [('prestashopextend_id', '=', str(external_id)),
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.prestashopextend_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.prestashopextend_id
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on prestashopextend! Prevent False values
