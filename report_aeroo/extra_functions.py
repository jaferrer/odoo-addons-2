...
class ExtraFunctions(object):
    ...
    def _xhtml_to_odt(self, xhtml):
        ...
        xhtml = u'<?xml version="1.0"?><html xmlns="http://www.w3.org/1999/xhtml"><head><title></title></head><body>' + \
                xhtml + u'</body></html>'
        ...
        return (u"<htmlparse>--key--" + tools.ustr(self.index_html) + u"--key--" + stra + u"</htmlparse>").replace('\n', ' ')
    ...
    def handle_img(self, full_tag, src, filename):
        ...
        if not os.path.exists(filename):
            raise exceptions.UserError("Image file name does not exist")
