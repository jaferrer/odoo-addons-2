...
def currency_to_text(sum, currency, language):
    ...
    >>> currency_to_text(123.12, 'LVL', 'ru_RU') ==
    'стo двaдцать три лата двенадцать сантимов'
    True

    >>> currency_to_text(123.12, 'USD', 'ru_RU') ==
    'стo двaдцать три доллара США двенадцать центов'
    True
    """
    ...
    if language == 'lv_LV':
        ...
        if sum_cur == 1 or (
                str(sum_cur)[-1] == '1' and str(sum_cur)[-2] != '1'):
            if currency == 'LVL':
                cur_in_words += u' lats'
            elif currency == 'EUR':
                cur_in_words += u' eiro'
        ...
        if sum_frc == 1 or (
                str(sum_frc)[-1] == '1' and str(sum_frc)[-2] != '1'):
            if currency == 'LVL':
    ...
    if language == 'ru_RU':
        ...
        if sum_cur == 1 or (
            ...
            sum_cur in [2, 3, 4] or
            (str(sum_cur)[-1] in ['2', '3', '4'] and str(sum_cur)[-2] != '1')
        ):
            if currency == 'LVL':
                cur_in_words += u' лата'
            elif currency == 'EUR':
            ...
            (sum_cur >= 5 and sum_cur <= 20) or
            str(sum_cur)[-1] not in [2, 3, 4]
        ):
            if currency == 'LVL':
        ...
        if sum_frc == 1 or (
            ...
            sum_frc in [2, 3, 4] or
            (str(sum_frc)[-1] in ['2', '3', '4'] and str(sum_frc)[-2] != '1')
        ):
            if currency == 'LVL':
            ...
            (sum_frc >= 5 and sum_frc <= 20) or
            str(sum_frc)[-1] not in [2, 3, 4]
        ):
            if currency == 'LVL':
...
def dtowords(sum_integers, language):
    ...
    >>> dtowords(12345, 'lv_LV').encode('utf-8') ==
        'divpadsmit tūkstoši trīs simti četrdesmit pieci'
    True

    >>> dtowords(123456, 'lv_LV').encode('utf-8') ==
        'simts divdesmit trīs tūkstoši četri simti piecdesmit seši'
    True
    ...
    >>> dtowords(12345, 'ru_RU').encode('utf-8') ==
    'двенадцать тысяч триста сорок пять'
    True

    >>> dtowords(123456, 'ru_RU').encode('utf-8') ==
        'стo двaдцать три тысячи четыреста пятьдесят шесть'
    True
    """
    diginwords = u''
    if sum_integers == 0:
...
def wordify(chunk, chunknr, language):
    ...
    if language == 'lv_LV':
        ...
        skaitlix = [
            u'nulle', u'vien', u'div', u'trīs', u'četr', u'piec',
            u'seš', u'septiņ', u'astoņ', u'deviņ']
        skaitli_teens = [
            u'desmit', u'vienpadsmit', u'divpadsmit', u'trīspadsmit',
            u'četrpadsmit', u'piecpadsmit', u'sešpadsmit', u'septiņpadsmit',
            u'astoņpadsmit', u'deviņpadsmit']
        ...
        skaitli_teens = [
            u'ten', u'eleven', u'twelve', u'thirteen', u'fourteen', u'fifteen',
        ...
        skaitli = [
            u'ноль', u'один', u'два', u'три', u'четыре', u'пять', u'шесть',
            u'семь', u'восемь', u'девять']
        skaitlix = [u'', u'один', u'двa', u'три', u'четыре', u'пять', u'шесть',
                    u'семь', u'восемь', u'девять']
        skaitli_teens = [
            u'десять', u'одиннадцать', u'двенадцать', u'тринадцать',
        ...
        skaitli = [
            u'ноль', u'один', u'два', u'три', u'чотири', u'п\'ять',
        ...
        skaitli_teens = [
            ...
            u'чотирнадцять', u'п\'ятнадцять', u'шістнадцять',
            u'сімнадцять', u'вісімнадцять', u'дев\'ятнадцять']
    ...
    if chunklength == 3:
        if digit1 == '1':
            if (
                language == 'lv_LV' or language == 'ru_RU' or
                language == 'uk_UA'
            ):
                words += daudzums[0]
            elif language == 'en_US':
    ...
    if chunklength > 1:
        ...
        if digit2 == '1':
            if (
                language == 'lv_LV' or language == 'en_US' or
                language == 'ru_RU' or language == 'uk_UA'
            ):
    ...
    if len(words) > 0:
        ...
        if digit3 == '1' and chunknr > 0:
            ...
            if language == 'lv_LV' or language == 'en_US':
                ...
                if (int(digit3) == 2 or int(digit3) ==
                        3 or int(digit3) == 4) and digit2 != '1':
                    if chunknr == 1:
