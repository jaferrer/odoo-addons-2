...
class AerooReport(report_sxw):
    ...
    def create_aeroo_report(
        ...
        data = basic.generate(**oo_parser.localcontext).render()
        data = self.html_fields_post_process(data, xfunc).getvalue()
    ...
    def html_fields_post_process(self, data, xfunc):
        xml = {}
        print(data)
        with zipfile.ZipFile(data) as zf:
            xml['content.xml'] = zf.read("content.xml")
            xml['styles.xml'] = zf.read("styles.xml")

        tab = xml['content.xml'].split('&lt;htmlparse&gt;')
        if len(tab) > 1:
            for it in tab[1:]:
                subtab = it.split('&lt;/htmlparse&gt;')
                sub = subtab[0].split('--key--')[2]
                substr = subtab[1][:30]
                balise = [subtab[1][m.start(0):m.end(0)] for m in re.finditer("[<]\/text[:][a-z]*[>]", substr)]
                balise = ''.join(balise)
                xml['content.xml'] = xml['content.xml'].replace(
                    '&lt;htmlparse&gt;' + subtab[0] +
                    '&lt;/htmlparse&gt;' + balise, balise + unescape(sub))

            xml = xfunc._add_styles(xml)

            data_cpy_io = StringIO()
            with zipfile.ZipFile(data) as zf:
                with zipfile.ZipFile(data_cpy_io, mode='w') as zfcpy:
                    for item in zf.infolist():
                        if item.filename not in ['content.xml', 'styles.xml']:
                            zr = zf.read(item.filename)
                            zfcpy.writestr(item, zr)
                        else:
                            zfcpy.writestr(item, xml[item.filename])
