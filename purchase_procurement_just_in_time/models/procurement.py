...
class ProcurementOrderPurchaseJustInTime(models.Model):
    ...
    def propagate_cancels(self):
        ...
        for procurement in self:
            if procurement.rule_id.action == 'buy' and procurement.purchase_line_id:
                ...
                procurement.remove_procs_from_lines()
        return super(ProcurementOrderPurchaseJustInTime,
                     self.with_context(do_not_update_qty=True,
                                       do_not_update_price=True,
                                       do_not_unlink_lines=True)).propagate_cancels()
        ...
        while procurements_to_run:
            ...
            if seller_ok and ignore_past_procurements:
                ...
                if suppliers:
                    ...
                    min_date = fields.Datetime.to_string(seller.schedule_working_days(seller_delay, dt.now()))
                else:
    ...
    def compute_which_procs_for_lines(self):
        ...
        for rec in self:
            domain_lines = [('order_id.state', 'not in', self.get_forbidden_order_states_for_proc_assignment()),
                            ('order_id.picking_type_id.default_location_dest_id', '=', rec.location_id.id),
    ...
    def get_purchase_line_procurements(self, first_proc, purchase_date, company, seller, order_by, force_domain=None):
        ...
        for procurement in possible_procurements_grouping_period:
            procurement_schedule_date = procurement._get_purchase_schedule_date()
            procurement_purchase_date = procurement._get_purchase_order_date(procurement_schedule_date)
    ...
    def get_corresponding_draft_order_main_domain(self, seller):
        ...
        main_domain = [('partner_id', '=', seller.id),
                       ...
                       ('picking_type_id.default_location_dest_id', 'child_of', self.location_id.id),
    ...
    def get_corresponding_draft_order_values(self, name, origin, seller, date_order, date_order_max):
        ...
        return {
            ...
            'picking_type_id': self.rule_id.picking_type_id.id,
            'currency_id': seller.property_product_pricelist and
                           seller.property_product_pricelist.currency_id.id or
                           self.company_id.currency_id.id,
            'date_order': date_order,
            'date_order_max': date_order_max,
            'company_id': self.company_id.id,
            'fiscal_position_id': seller.property_account_position_id and
                                  seller.property_account_position_id.id or False,
            'payment_term_id': seller.property_supplier_payment_term_id.id or False,
    ...
    def group_procurements_by_orders(self):
        ...
        while procurements_to_check:
            ...
            seller = self.env['procurement.order']._get_product_supplier(first_proc)
            company_supplierinfo = first_proc.product_id.product_tmpl_id. \
                get_main_supplierinfo(force_company=first_proc.company_id)
            schedule_date = first_proc._get_purchase_schedule_date()
            purchase_date = first_proc._get_purchase_order_date(schedule_date)
            ...
            if draft_order and pol_procurements:
                line_vals = first_proc._prepare_purchase_order_line(draft_order, company_supplierinfo)
        ...
        for order_id in dict_lines_to_create.keys():
            for product_id in dict_lines_to_create[order_id].keys():
                ...
                for procurement in pol_procurements:
                    ...
                    if procurement == last_proc:
                        line.update_qty_price()
    ...
    def run(self, autocommit=False):
        ...
        for proc in self:
            if proc.rule_id and proc.rule_id.action == 'buy':
                if not self.env['procurement.order']._get_product_supplier(proc):
                    proc.set_exception_for_procs()
                if proc.state == 'running' and not proc.purchase_line_id and proc.state != 'exception':
