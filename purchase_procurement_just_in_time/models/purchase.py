...
class PurchaseOrderJustInTime(models.Model):
    ...
    def _prepare_order_line_move(self, order, order_line, picking_id, group_id):
        ...
        if order_line.taxes_id:
            taxes = order_line.taxes_id.compute_all(price_unit, order.currency_id, 1.0, order_line.product_id,
                                                    order.partner_id)
            price_unit = taxes['total_included']
        ...
        res = []
        if order.default_location_dest_id_usage == 'customer':
        ...
        move_template = {
            ...
            'date_expected': order_line.date_planned[:10] + ' 01:00:00',
    ...
    def button_cancel(self):
        ...
        res = super(PurchaseOrderJustInTime, self.with_context(cancel_procurement=True)).button_cancel()
        running_procs.remove_procs_from_lines(unlink_moves_to_procs=True)
        return res

    @api.multi
    def set_procs_to_exception_if_needed(self):
...
class PurchaseOrderLineJustInTime(models.Model):
    ...
    def create(self, vals):
        ...
        states_confirmed = states_with_moves + ['to approve']
    ...
    def act_windows_view_graph(self):
        ...
        if warehouse:
            ...
            raise UserError(_("Your company does not have a warehouse"))
