...
class HrHolidays(models.Model):
    ...
    def add_timesheet_line(self, description, date, hours, account):
        ...
        Update by NDP to extract the analytic dict value in _populate_data_for_analytic_line
        """
        self.ensure_one()
        ...
        self.sudo().with_context(force_write=True).write({
            'analytic_line_ids': [
                (0, False, self._populate_data_for_analytic_line(description, date, hours, projects[0]))
            ]
        })

    @api.multi
    def _populate_data_for_analytic_line(self, description, date, hours, project):
        self.ensure_one()
        return {
            'name': description,
        ...
        for leave in self:
            ...
            for index_day in range(0, int(round(leave.number_of_days_temp))):
                ...
                leave.add_timesheet_line(
                    description=u" %s %.1f jour(s)" % (leave.holiday_status_id.name, abs(leave.number_of_days)),
                    date=dt_current,
                    hours=unit_amount,
                    account=account,
                )
