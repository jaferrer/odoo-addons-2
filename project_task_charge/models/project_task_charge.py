...
class ProjectTaskCharge(models.Model):
    ...
    def get_select_data(self):
        ...
        count(nb_days.date)                                                  AS duration,
        (td.planned_hours / count(nb_days.date))                             AS duration_per_day,
        (td.price_subtotal / td.planned_hours *  (td.planned_hours / count(nb_days.date)))
                                                                             AS sale_order_line_amount_per_day,
        ...
        coalesce(((td.planned_hours / count(nb_days.date)) / 7 * 100), 0)    AS percent_duration,
        coalesce((hs.spent_by_day / (td.planned_hours / count(nb_days.date)) * 100), 0)
                                                                             AS percent_advancement
        """
 ...
 CREATE OR REPLACE VIEW project_task_charge AS (
    WITH max_task_date AS (
        SELECT NOW() + interval '1 month' * (select value::INT from ir_config_parameter where
                                            key = 'project_task_charge.calc_max_date') as max_date
    ),
     min_task_date AS (
        SELECT NOW() - interval '1 month' * (select value::INT from ir_config_parameter where
                                             key = 'project_task_charge.calc_min_date') as min_date
     ),
     days AS (
