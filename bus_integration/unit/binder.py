...
@BUSEXTEND
class BusextendBindingModelBinder(BusextendBinding):
    """
    ...
    def to_backend(self, record_id, models, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.external_key
            return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.external_key
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on magentoextend! Prevent False values
