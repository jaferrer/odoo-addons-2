...
from openerp.tools import frozendict, lazy_property, ormcache, Collector
                    ...
                    if f_obj_type:
                        ...
                        if f_pg_type == 'varchar' and f._type in ('char', 'selection') and f_pg_size and (
                            ...
                            try:
                                ...
                                cr.execute(
                                    'ALTER TABLE "%s" RENAME COLUMN "%s" TO temp_change_size' % (
                                    self._table, k))
                                cr.execute(
                                ...
                                cr.execute('UPDATE "%s" SET "%s"=temp_change_size::%s' % (
                                    self._table, k, pg_varchar(f.size)))
                                cr.execute(
                                    'ALTER TABLE "%s" DROP COLUMN temp_change_size CASCADE' % (self._table,))
                            cr.commit()
                            ...
                            if (f_pg_type == c[0]) and (f._type == c[1]):
                                if f_pg_type != f_obj_type:
                                    ...
                                    try:
                                        with cr.savepoint():
                                            cr.execute('ALTER TABLE "%s" ALTER COLUMN "%s" TYPE %s' % (
                                                self._table, k, c[2]), log_exceptions=False)
                                    except psycopg2.NotSupportedError:
                                        ...
                                        cr.execute(
                                            'UPDATE "%s" SET "%s"= __temp_type_cast%s' % (self._table, k, c[3]))
                        ...
                        if f_pg_type != f_obj_type:
                            if not ok:
                                ...
                                cr.execute(
                                    'ALTER TABLE "%s" RENAME COLUMN "%s" TO "%s"' % (self._table, k, newname))
                                cr.execute(
                        ...
                        cr.execute("SELECT indexname FROM pg_indexes WHERE indexname = %s and tablename = %s",
                                   (indexname, self._table))
                        res2 = cr.dictfetchall()
                        if not res2 and f.select:
                            cr.execute(
                                'CREATE INDEX "%s_%s_index" ON "%s" ("%s")' % (self._table, k, self._table, k))
                            cr.commit()
                            if f._type == 'text':
