...
class SaleOrderRelance(models.Model):
    ...
    report_id = fields.Many2one('ir.actions.report', string=u"Report", related='dunning_type_id.report_id',
                                readonly=True)
...
class SaleOrder(models.Model):
    ...
    sale_order_dunning_ids = fields.Many2many('sale.order.dunning', string="Dunnings")
    dunning_number = fields.Integer("Number of Dunning send", compute='_compute_dunning_number')
    ...
    def _validate_to_create_dunning(self):
        if self.state != 'sent':
            raise osv.osv.except_orm(_(u"Error !"), _(u"You can't create a Dunning on a sale order which is not sent"))
