...
class StockPicking(models.Model):
    ...
    location_id_compute = """SELECT sm.location_id
FROM stock_move sm
WHERE sm.picking_id = $1.id
LIMIT 1"""

    location_dest_id_compute = """SELECT sm.location_dest_id
FROM stock_move sm
WHERE sm.picking_id = $1.id
LIMIT 1"""

    location_id_sql = fields.Many2one('stock.location', string=u"Location", readonly=True,
                                      compute_sql=location_id_compute)
    location_dest_id_sql = fields.Many2one('stock.location', string=u"Destination Location", readonly=True,
                                           compute_sql=location_dest_id_compute)
    location_id = fields.Many2one('stock.location', related='location_id_sql')
    location_dest_id = fields.Many2one('stock.location', related='location_dest_id_sql')
