...
class ProjectTaskTypeRel(models.Model):
    _inherit = 'project.task.type.rel'

    @api.multi
    def get_values_new_task(self, task, project):
        result = super(ProjectTaskTypeRel, self).get_values_new_task(task, project)
    ...
    def synchronize_default_tasks(self):
        result = super(ProjectTaskTypeRel, self).synchronize_default_tasks()
        for rec in self:
            for generated_task in result[rec.type_id]:
                ...
                for previous_template_task in previous_template_tasks:
                    previous_tasks |= previous_template_task.find_generated_task_for_template(rec.project_id.id)
                for next_template_task in next_template_tasks:
                    next_tasks |= next_template_task.find_generated_task_for_template(rec.project_id.id)
                ...
                if template_parent_task:
                    parent_task = template_parent_task.find_generated_task_for_template(rec.project_id.id)
                    if parent_task:
