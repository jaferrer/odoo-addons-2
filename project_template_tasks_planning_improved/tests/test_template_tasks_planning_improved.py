...
class TestTemplateTasksPlanningImproved(common.TransactionCase):
    def setUp(self):
        ...
        self.stage2 = self.browse_ref('project.project_stage_2')
        self.stage3 = self.browse_ref('project.project_stage_3')
    ...
    def test_10_template_tasks_planning_improved(self):
        ...
        self.assertEqual(len(task1.next_task_ids), 2)
        ...
        task8 = self.env['project.task'].search(domain_stage3 + [('generated_from_template_id', '=', self.template_task8.id)])
        task9 = self.env['project.task'].search(domain_stage3 + [('generated_from_template_id', '=', self.template_task9.id)])
        self.assertEqual(len(parent_task_0), 1)
        ...
        self.assertFalse(task8.previous_task_ids)
        self.assertFalse(task8.next_task_ids)
        self.assertFalse(task9.previous_task_ids)
        self.assertFalse(task9.next_task_ids)
        ...
        task6 = self.env['project.task'].search(domain_stage2 + [('generated_from_template_id', '=', self.template_task6.id)])
        task7 = self.env['project.task'].search(domain_stage2 + [('generated_from_template_id', '=', self.template_task7.id)])
        ...
        self.assertEqual(len(task6.previous_task_ids), 2)
        self.assertIn(task4, task6.previous_task_ids)
        self.assertIn(task5, task6.previous_task_ids)
        self.assertEqual(len(task6.next_task_ids), 2)
        self.assertIn(task8, task6.next_task_ids)
        self.assertIn(task9, task6.next_task_ids)
        self.assertFalse(task7.previous_task_ids)
        self.assertEqual(len(task7.next_task_ids), 2)
        self.assertIn(task8, task7.next_task_ids)
        self.assertIn(task9, task7.next_task_ids)
