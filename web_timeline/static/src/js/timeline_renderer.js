odoo.define('web_timeline.TimelineRenderer', function (require) {
    ...
    var TimelineRenderer = AbstractRenderer.extend({
        ...
        init: function (parent, state, params) {
            ...
            this.overlap_field = params.overlap_field;
        ...
        _onScaleDayClicked: function () {
            this._scaleCurrentWindow(1, 'day');
        },
        ...
        _onScaleWeekClicked: function () {
            this._scaleCurrentWindow(1, 'week');
        },
        ...
        _onScaleMonthClicked: function () {
            this._scaleCurrentWindow(1, 'month');
        },
        ...
        _onScaleYearClicked: function () {
            this._scaleCurrentWindow(1, 'year');
        },
         ...
         * @private
         */
        _scaleCurrentWindow: function (factor, unit='hours') {
            if (this.timeline) {
                this.current_window = this.timeline.getWindow();
                this.current_window.end = moment(this.current_window.start).add(factor, unit);
                this.timeline.setWindow(this.current_window);
            }
        },
            ...
            if (this.mode) {
                ...
                switch (this.mode) {
                    ...
                    case 'custom':
                        ...
                        start = pyeval.py_eval(this.view.exprCustomStart, pyeval.context())
                        end = pyeval.py_eval(this.view.exprCustomEnd, pyeval.context())
                }
                if (end && start) {
        ...
        init_timeline: function () {
            ...
            this._load_data(this.modelClass.last_params)
        ...
        _load_data: function(params, previous_grp_result={}, reset=true){
            if(reset){
                this.visData.clear();
            ...
            return this.modelClass._read_group(params).then(result => {
                return $.when.apply($,
                    result.flatMap(grouping => {
                        const grp_result = this._populate_vis_group(grouping, params, previous_grp_result);
                        if (grouping.__context && grouping.__context.group_by){
                            return this._load_data({
                                ...params,
                                groupBy:  grouping.__context.group_by,
                                domain:  [...params.domain || [], ...grouping.__domain || []],
                            }, grp_result, false)
                        }
                        return this.modelClass._do_search({...params, domain: grouping.__domain}).then(events => {
                            return events.filter(evt => evt[this.date_start]).map(evt => {
                                const el = this.convert_data_to_timeline(evt, params);
        ...
        _populate_vis_group: function (grouping, params, previous_grp_result) {
            let grouped_on = params.groupBy
            const level = previous_grp_result.level || 0
            if (Array.isArray(params.groupBy)) {
                grouped_on = params.groupBy[0]
            }
            let value = grouping[grouped_on];
            let label = value;
            ...
            if (!this.visGroups.get(grp_id)) {
                ...
                if (grouped_on === _.last(params.groupBy)) {
                    ...
                    if (this.displayNumberOnGroup) {
                        current_group.content += " (" + grouping.__count + ")"
                    }
                }
                this.visGroups.add(current_group);
            }
            if (previous_grp_result.id) {
                const previous_vis_group = this.visGroups.get(previous_grp_result.id);
                this.visGroups.update({
                    id: previous_grp_result.id,
                    nestedGroups: [...previous_vis_group.nestedGroups || [], grp_id]
                })
            }

            return {"field": grouped_on, "id": grp_id, "level": level + 1};
        },
        ...
        convert_data_to_timeline(evt, params) {
            ...
            if (params.groupBy) {
                visEvt.group = params.groupBy.map(grp => grp + '_' + this._get_id(evt[grp])).join("_")
            }
        ...
        _get_color_for_item(evt) {
            if (evt.hasOwnProperty(this.view.color_field)) {
                return evt[this.view.color_field];
            }
            let color;
            ...
            return this.view.defaultColor;
        },
        _get_type_for_item(evt) {
            if (this.view.event_type && evt.hasOwnProperty(this.view.event_type)) {
                return evt[this.view.event_type];
            }
            const context = _.extend({}, evt, {
                uid: session.uid,
                current_date: moment().format('YYYY-MM-DD')
            });
            for (const [type, expr] of this.view.elementType) {
                if (py.PY_isTrue(py.evaluate(expr, context))) {
                    return type;
                }
            }
            return this.view.defaultType
        },
