odoo.define('web_timeline_improved.TimelineView', function (require) {
    ...
    TimelineView.include({
        ...
        _get_option_timeline: function () {
            ...
            if (Boolean(this.fields_view.arch.attrs.hide_weekend) == true) {
                var date = new Date();
                var saturday = new moment(new Date(date.setDate(date.getDate() - (date.getDay() - 1) + 5)));
                var sunday = new moment(new Date(date.setDate(date.getDate() - (date.getDay() - 1) + 6)));
                options.hiddenDates.push(
                    {
                        start: saturday.format("YYYY-MM-DD 00:00:00"),
                        end: saturday.add(1, 'day').format("YYYY-MM-DD 00:00:00"),
                        repeat: 'weekly',
                    },
                    {
                        start: sunday.format("YYYY-MM-DD 00:00:00"),
                        end: sunday.add(1, 'day').format("YYYY-MM-DD 00:00:00"),
                        repeat: 'weekly',
                    },
                );
            }
