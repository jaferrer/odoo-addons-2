...
class AccountMoveExportWizard(models.TransientModel):
    ...
    def _mark_move_lines_as_exported(self, move_lines):
        ...
        """
        move_lines.mapped('line_id').write({'exported': True})

    @api.multi
    def _get_move_by_dates_and_journals(self, date_from, date_to, journals):
        """ Get all account.move in selected journals in a date frame
    ...
    def _get_unexported_moves(self, journals):
        """ Get all account.move in the selected journals that have not already been exported
    ...
    def _create_export_from_move(self, moves):
        ...
        :param moves: recordset of account.move to export
